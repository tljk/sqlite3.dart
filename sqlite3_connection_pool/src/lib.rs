...
/// `initializer`. The uninitialized pool must be completed with either a call to
/// [pkg_sqlite3_connection_pool_initialize] or to
/// [pkg_sqlite3_connection_pool_close_uninitialized].
///
/// The name pointer must be kept alive until this function returns a client, or the initializer is
/// used (droppd or turned into a client).
#[unsafe(no_mangle)]
